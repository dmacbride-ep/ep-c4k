locals {
  db-hostname          = "${azurerm_mysql_server.database.fqdn}"
  db-port              = "3306"
  db-schema-name       = "ElasticPathDB${random_string.schema_suffix.result}"
  db-connection-params = "?characterEncoding=UTF-8&useSSL=true&pinGlobalTxToPhysicalConnection=true&useLocalSessionState=true"
}

resource "kubernetes_secret" "mysql_server_secret" {
  depends_on = [
  ...
  data = {
    ...
    EP_DB_ROOT_USER            = "epadmin-${random_string.root_username_suffix.result}@${var.database_name}"
    ...
    EP_DB_USER        = "epuser-${random_string.nonroot_username_suffix.result}@${var.database_name}"
    EP_DB_USER_SHORT  = "epuser-${random_string.nonroot_username_suffix.result}"
